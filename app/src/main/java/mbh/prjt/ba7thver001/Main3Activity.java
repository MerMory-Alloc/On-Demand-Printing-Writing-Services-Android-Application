...
public class Main3Activity extends AppCompatActivity implements ProfileFragment.SendPhotoProfile {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        usersRef=database.getReference("Users/Admins");
        adminRef=usersRef.child(mAuth.getCurrentUser().getUid()).getRef();
        ...
        adminRef.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                admin = snapshot.getValue(Admin.class);
                    if (admin != null) {
                        openFirstFragment(DashboardFragment.newInstance(admin.getSrvcProviderId()));
    ...
    protected void onStart(){
        ...
        bottomNavigation.setOnNavigationItemSelectedListener(item -> {
            ...
            switch (item.getItemId()) {
                case R.id.profile:
                    openFragment(ProfileFragment.newInstance(admin));
                    return true;
                case R.id.dashboard:
                    openFirstFragment(DashboardFragment.newInstance(admin.getSrvcProviderId()));
                    return true;
                case R.id.notification:
                    openFragment(NotificationFragment.newInstance(notificationArrayList));
                    openNote(keys.size());
                    return true;
            }
    ...
    @Override
    public void SendPhotoProfile(Admin adminNew) {
        HashMap map=new HashMap();
        map.put("srvcProviderProfilImage",adminNew.getSrvcProviderProfilImage());
